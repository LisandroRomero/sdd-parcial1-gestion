...
El sistema SHALL exponer `DELETE /api/v1/ingredientes/{id}` que aplica soft delete asignando `deleted_at = now()`. El ingrediente desaparece de listados pero sus asociaciones históricas a `ProductoIngrediente` se preservan. Requiere rol ADMIN o STOCK.
...
#### Scenario: El ingrediente eliminado no aparece en listados
...
El sistema SHALL garantizar que no existan dos ingredientes activos con el mismo nombre (case-sensitive). La validación ocurre en el `IngredienteService` antes de delegar al repositorio.
