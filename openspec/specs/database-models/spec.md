...
El sistema SHALL definir el modelo Ingrediente en `backend/ingredientes/model.py` con nombre VARCHAR(100) UNIQUE, es_alergeno BOOLEAN default False, created_at.

#### Scenario: Ingrediente tiene nombre único
- **WHEN** se define Ingrediente
- **THEN** SHALL tener nombre único, es_alergeno booleano, y created_at
